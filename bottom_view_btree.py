...
def store_bottom_view(root, recd, curr_dis, level):
    ''' 
    ...
    store_bottom_view(root.left, recd, curr_dis-1, level+1)

    # recurse for right subtree
    store_bottom_view(root.right, recd, curr_dis+1, level+1)


def print_bottom_view(recd):
    '''
    ...
    store_bottom_view(root, recd, 0, 0)

    ans = []
    for value in sorted(recd):
        ans.append(recd[value][0])
